...
import { CAPTURA_COLUMNS } from "@/modules/captura/plantConfig";
...
function safeJsonParse(raw: string): VisionPayload {
  ...
  try {
    ...
    // Respuestas con bloque markdown: ```json ... ```
...
export async function POST(req: Request) {
  try {
    ...
    const columnKeys = CAPTURA_COLUMNS.map((c) => c.key);

    const prompt = [
      "Eres un extractor OCR experto de reportes de produccion.",
      "Analiza la imagen del reporte y devuelve SOLO JSON valido (sin markdown, sin texto extra).",
      ...
      "- No inventar datos.",
      "- Detecta filas de la tabla principal que contengan datos productivos.",
      "- Si hay datos de cabecera como fecha/planta/maquina/capturista, replica esos valores en cada fila cuando aplique.",
