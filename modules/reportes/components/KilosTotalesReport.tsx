...
import {
  getKilosReport,
  type KilosReportMode,
  type KilosMaquinaGroup,
  type KilosAggregatedRow,
} from "../kilosReportActions";
...
function kilosToExcelSheets(
  ...
  if (daily && daily.length > 0) {
    ...
    for (const mg of daily) {
      for (const ng of mg.noMaquinaGroups) {
        ...
        rows.push(["", "", "", "", "Subtotal no. máquina", "", ng.subtotalKilos, ""]);
      }
      rows.push(["", "", "", "", `Total ${mg.maquina}`, "", mg.totalKilos, ""]);
    }
    rows.push([
      "",
      "",
  ...
  if (aggregated && aggregated.length > 0) {
    const head = ["Máquina", "No. máquina", "Total kg"];
    const rows: (string | number)[][] = [
      head,
      ...aggregated.map((r) => [r.maquina, r.noMaquinaLabel, r.totalKilos]),
...
interface KilosTotalesReportProps {
  planta: PlantaValue;
}

export function KilosTotalesReport({ planta }: KilosTotalesReportProps) {
  const [mode, setMode] = useState<KilosReportMode>("day");
  ...
  const [daily, setDaily] = useState<KilosMaquinaGroup[] | null>(null);
  const [aggregated, setAggregated] = useState<KilosAggregatedRow[] | null>(null);
  ...
  const runReport = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await getKilosReport(
        planta,
        mode,
  ...
  const grandTotalDaily = useMemo(() => {
    if (!daily) return 0;
    return daily.reduce((s, g) => s + g.totalKilos, 0);
  }, [daily]);

  const grandTotalAgg = useMemo(() => {
    if (!aggregated) return 0;
    return aggregated.reduce((s, r) => s + r.totalKilos, 0);
  }, [aggregated]);
  ...
  const exportToExcel = useCallback(() => {
    const sheets = kilosToExcelSheets(daily, aggregated, grandTotalDaily, grandTotalAgg);
    if (sheets.length === 0) return;
    downloadExcelWorkbook(`reporte-kilos-${planta}-${exportLabelSuffix}`, sheets);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-2xl font-semibold text-strong">Kilos totales</h1>
              <p className="text-sm text-subtle">
                Suma de <span className="text-mute">peso_total</span> por máquina · Planta:{" "}
      ...
      <ReportQueryToolbar
        mode={mode}
        onModeChange={(m) => setMode(m as KilosReportMode)}
        day={day}
        onDayChange={setDay}
        monthYm={monthYm}
            ...
            <table className="ui-table min-w-[920px]">
              <thead>
                <tr>
                  ...
                  <th className="px-3 py-2.5 text-right">Peso total (kg)</th>
