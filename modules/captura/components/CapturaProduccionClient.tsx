...
/** Exportación: todo lo de BD + ediciones locales sin guardar + filas nuevas sin id. */
function mergeProduccionDbExportWithLocal(
  fromDb: ProduccionRowState[],
  localRows: ProduccionRowState[]
): ProduccionRowState[] {
  const clientById = new Map<number, ProduccionRowState>();
...
export function CapturaProduccionClient({
  ...
  const exportToExcel = useCallback(async () => {
    ...
    try {
      ...
      const toExport = mergeProduccionDbExportWithLocal(res.rows, rows);
      if (toExport.length === 0) {
        setSaveBanner("No hay filas para exportar.");
        return;
      }
      const headers = ["No. registro", ...CAPTURA_COLUMNS.map((c) => c.label)];
      ...
      for (const row of toExport) {
        ...
        aoa.push([
          nr != null ? nr : "",
          ...CAPTURA_COLUMNS.map((c) => row.values[c.key] ?? ""),
        ]);
      }
      const stamp = new Date().toISOString().slice(0, 10);
      downloadExcelWorkbook(`captura-produccion-${planta}-${stamp}`, [
        { name: "Captura", rows: aoa },
      ]);
    } finally {
      ...
      void (async () => {
        try {
          ...
          setRows((prevRows) =>
            ensureTrailing(
              orderRowsWithEmptyFechaLast(
                mergeProduccionDbExportWithLocal(res.rows, prevRows)
              )
            )
          );
  ...
  const reloadFromDb = useCallback(async () => {
    ...
    try {
      if (needsFullDatasetRef.current) {
        ...
        setRows((prev) =>
          ensureTrailing(
            orderRowsWithEmptyFechaLast(mergeProduccionDbExportWithLocal(res.rows, prev))
          )
        );
        return;
      }
          ...
          >
            ...
            <tbody>
              {displayRows.length === 0 ? (
                <tr>
                  <td
                    colSpan={CAPTURA_COLUMNS.length + 1}
